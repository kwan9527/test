...
public class MP4Activity extends Activity implements OnClickListener, MountListener.OnStateChangedListener {
	...
	private void initUI() {
		if (FunctionApplication.isAutoMode)
			videoType = "."
					+ XmlUtils.getEntity(this.getClass().getSimpleName().toString()).getParameter().toString().trim();
	...
	public void onClick(View v) {
		...
		case R.id.button_pass:
			...
			if (FunctionApplication.isAutoMode)
				jumpUtils.jumpTo(getResources().getString(R.string.title_activity_mp4), MP4Activity.this,
	...
	public boolean findVideo(String mnt, String type) {
		...
		try {
			...
			for (int i = 0; i < fl.length; i++) {
				if (fl[i].isDirectory())
					findVideo(fl[i].getAbsolutePath(), type);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			new AlertDialog.Builder(MP4Activity.this).setTitle(R.string.title_tips)
						...
						public void onClick(DialogInterface dialog, int whichButton) {
							...
							if (FunctionApplication.isAutoMode)
								jumpUtils.jumpTo(getResources().getString(R.string.title_activity_mp4),
										MP4Activity.this, XmlUtils.activityClasses.get(0), false);
							MP4Activity.this.finish();
						}
					}).show();
			return true;
